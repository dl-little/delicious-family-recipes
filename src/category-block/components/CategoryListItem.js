...
export default function CategoryListItem( props ) {
	...
	const { id, name, meta } = category;
	const { dfr_image } = meta;
	const imageObj = !! dfr_image.length
		? useSelect( ( select ) => {
	...
	const renderCategoryImage = ( imageObj ) => {
		if ( ! imageObj.media_details || ! imageObj.media_details?.sizes ) {
			return;
		}

		const src = getSource( imageObj.media_details.sizes );
		return <img className="cat-img" src={ src } />;
	};

	return (
		<li key={ id } className={ `cat-item cat-item-${ id }` }>
			<a href="#" className="cat-link">
				{ !! imageObj && renderCategoryImage( imageObj ) }
				<span className="cat-span">{ renderCategoryName( name ) }</span>
			</a>
		</li>
	);
}
